"""
Comprehensive Feature Tuning - Sweep through many more parameters
...
def run_comprehensive_feature_tuning():
    ...
    try:
        wandb.init(
            ...
            group="comprehensive-feature-tuning",
            name=f"comprehensive-tuning-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
            config={
                "epochs": 12,
                "batch_size": 32,
                "learning_rate": 0.01,
    ...
    # Save to YAML
    output_path = "configs/comprehensive_feature_tuning.yaml"
    ...
    print(f"⏱️  Total time: {sum([r['time_taken'] for r in results])/60:.1f} minutes")
    ...
    for i, result in enumerate(results[:10], 1):
        print(f"{i:2d}. {result['config_name']}: {result['roc_auc']:.4f}")
